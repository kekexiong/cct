									...
									<div class="row norow">
														...
														<div class="form-group">
															<label for="span-medium-2"
																class="control-label col-md-4 text-right">银行返回信息:</label>
															<div class="col-md-8 paddingnone">
																<input name="backMsg"
																	id="BACK_MSG" class="form-control"
																	placeholder="银行返回信息">
															</div>
														</div>
													</div>
							...
							<div class="portlet-title">
								<div class="caption">
									<i class="fa fa-search font-blue"></i> <span
										class="caption-subject font-blue bold uppercase">代码自动生成信息列表</span>
								</div>
								<div class="tools">
	                       	...
	                       	<input type="hidden" value = "" id = "hiddenUUid"/>
										...
										<div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
											<div class="form-group">
												...
												<div class="col-md-8 paddingnone">
													<input name="backMsg"
														id="BACK_MSG_SHOW" class="form-control"
														placeholder="银行返回信息">
												</div>
											</div>
										</div>
    	...
    	<!-- ---------------------------------------详情页面展示(获取输入值的输入框后缀统一增加_VIEW标识符)------------------------------------------------------------ -->
			<div class="modal fade" id="viewWin" tabindex="-1" role="dialog" data-backdrop="static" data-width="700px" data-height="300px">
		        <div class="modal-header">
		           ...
		           <h4 class="modal-title" >代码自动生成详情信息</h4>
		        </div>
		        <div class="modal-body">
		           <div class="portlet-body form">
		                       ...
		                       <div class="row norow">
											...
											<div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
												<div class="form-group">
													...
													<div class="col-md-8 paddingnone">
														<input name="backMsg"
															id="BACK_MSG_VIEW" class="form-control" disabled="disabled"
															placeholder="银行返回信息">
													</div>
												</div>
											</div>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$("#edit_btn").on('click', function() {
				...
				var title = document.getElementById("addOrUpdateWin_title");
				title.innerHTML = "代码自动生成修改信息";
	    		addOrUpdate("update",record);
			});
	    	//添加保存按钮
	    	var param = {};
			$("#addform_save_btn").off('click').on('click', function() {
							param.backMsg = $("#BACK_MSG_SHOW").val();
				var hiddenType = $("#hiddenType").val();
				var url = "";
				if("1" == hiddenType){
					url = baseURL + "/SES/TSesBankBkMsg/TSesBankBkMsgsave";
				}else if ("2" == hiddenType){
					url = baseURL + "/SES/TSesBankBkMsg/TSesBankBkMsgupdate";
				...
				if(selectRocords.length <= 0){
					...
					bootbox.confirm("选择了"+selectRocords.length+"条, 确认删除?", function(truthBeTold){
						if(truthBeTold){
		    				 $.ajax({
		     	                type: "POST",
		     	                url: baseURL + "/SES/TSesBankBkMsg/TSesBankBkMsgdeleteByUuid",
		     	                data:{
		     	                	uuids:uuids
		     	                },
		     	               contentType: "application/x-www-form-urlencoded;charset=utf-8",
		...
		function addOrUpdate(type,record){
			if("add" == type){
				...
				$.ajax({
					type : "POST",
					url : baseURL + "/SES/TSesBankBkMsg/TSesBankBkMsggetDetail",
					data :param,
					contentType: "application/x-www-form-urlencoded;charset=utf-8",
					dataType : "json",
					success:function(data){
					<!--修改页面展示字段-->
								document.getElementById('BACK_MSG_SHOW').value=data.data.backMsg;
		...
		var queryInner_Path = baseURL + "/SES/TSesBankBkMsg/TSesBankBkMsgquery";
		//实现查询按钮方法
		function reloadGrid(){
			$('table > tbody').empty();
	    	if(limit==0) {
		...
		function view(uuid){
			...
			$.ajax({
				...
				dataType : "json",
				success:function(data){
							document.getElementById('BACK_MSG_VIEW').value=data.data.backMsg;
					$("#viewWin").modal('show');
				...
				if (file.lastIndexOf('.') != -1) {
					...
					if(ext =="xlsx"){
						...
						$("#importOpenWinform").ajaxSubmit({
			                type: 'post',
						    url : baseURL +'/SES/TSesBankBkMsg/TSesBankBkMsgimportExcel',
						    dataType: "json",
			                success: function(data){
		...
		$("#errorImExcel").off('click').on('click', function() {
			...
			window.open(baseURL + "/SES/TSesBankBkMsg/TSesBankBkMsggetFailExport");
		...
		$("#isExport_btn").on('click', function() {
			...
			$("#exportform_save_btn").off('click').on('click', function() {
				window.open(baseURL +"/SES/TSesBankBkMsg/TSesBankBkMsgExport");
