...
<div style="maring:5px 5px 10px 5px;"><h5 style="font-weight:bold;" >状态操作记录</h5></div>
		    ...
		    <!-- 主体-->
		        ...
		        <div class="row">
		                ...
		                <div class="portlet light bordered scaffold-list">
		                        ...
		                        <table  class="table table-striped table-bordered table-hover table-checkable order-column" id="queryMecGrid">
		                            <thead>
		                                <tr>
		                                    ...
		                                    <th class="nowrap"> 操作前状态 </th>
		                                    <th class="nowrap"> 操作动作</th>
		                                    <th class="nowrap"> 操作员</th>
		                                    <th class="nowrap"> 操作时间</th>
		                                    <th class="nowrap"> 原因</th>
		                                    <th class="nowrap"> 操作渠道</th>
		                                </tr>
		                            </thead>
		                            <tbody>
		                            </tbody>
		                    ...
		                    <!-- 分页开始 -->
		                    <div class="row no-margin" id="pagination-bar"  style="margin-left:10px" style="background-color:white;">
		...
		$.ajax({
            ...
            success: function(data) {
                ...
                $.each(data.items, function(index, obj) {  
                	...
                	tr.append($("<td class=\"nowrap\"></td>").text(obj.mnoNm));
                	
                	if(obj.sts == '00'){
                		tr.append($("<td class=\"nowrap\"></td>").text('开通'));
    				}else if(obj.sts == '01'){
    					tr.append($("<td class=\"nowrap\"></td>").text('关闭'));
    				}else{
                	...
                	tr.append($("<td class=\"nowrap\"></td>").text(obj.cteStffNo));
                	tr.append($("<td class=\"nowrap\"></td>").text(obj.dtCte.substring(0,4)+"-"+obj.dtCte.substring(4,6)+"-"+obj.dtCte.substring(6,8)+" "+obj.tmCte.substring(0,2)+":"+obj.tmCte.substring(2,4)+":"+obj.tmCte.substring(4,6)));
                	tr.append($("<td class=\"nowrap\"></td>").text(obj.rmk));
                	var sysId = obj.sysId;
                	if(sysId == "WX" || sysId == "wx"){
                		sysId="微信";
                	}else if(sysId == "agent" || sysId == "AGENT"){
                		sysId="代理商";
                	}
                	tr.append($("<td class=\"nowrap\"></td>").text(sysId||''));
