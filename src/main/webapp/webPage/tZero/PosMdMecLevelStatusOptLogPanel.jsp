...
<body class="page-header-fixed page-sidebar-closed-hide-logo page-content-white page-md" style="background-color:white;">
	<div style="maring:5px 5px 10px 5px;"><h5 style="font-weight:bold;" >等级状态操作记录</h5></div>
    <!-- BEGIN HEADER -->
   <%--  <jsp:include page="../common/header.jsp" flush="true"/> --%>
		                ...
		                <div class="portlet light bordered scaffold-list">
		                    <div class="portlet-body"  >
		                        <table  class="table table-striped table-bordered table-hover table-checkable order-column" id="queryMecGrid">
		                            <thead>
		                                <tr>
		                                	<th class="nowra p">序号</th>
											<th class="nowrap">商编</th>
											<th class="nowrap">商户名称</th>
											<th class="nowrap">原等级状态</th>
											<th class="nowrap">变更等级状态</th>
											<th class="nowrap">操作员</th>
											<th class="nowrap">操作来源</th>
    ...
    function reloadGrid() {
    	...
    	start=(page-1)*limit;
		var param = {page:page,start:start,limit:limit,optTyp:optTyp};
		...
		$.ajax({
            ...
            success: function(data) {
                ...
                $.each(data.items, function(index, obj) {  
                	...
                	if(obj.beforeLevelStatus=='01'){
                		tr.append($("<td class=\"nowrap\"></td>").text('可升可降'));
                	}else if(obj.beforeLevelStatus=='02'){
                		tr.append($("<td class=\"nowrap\"></td>").text('只升不降'));
                	}else if(obj.beforeLevelStatus=='00'){
                		tr.append($("<td class=\"nowrap\"></td>").text('不升不降'));
                	}else{
                		tr.append($("<td class=\"nowrap\"></td>").text(''));
                	}
                	
                	if(obj.afterLevelStatus=='01'){
                		tr.append($("<td class=\"nowrap\"></td>").text('可升可降'));
                	}else if(obj.afterLevelStatus=='02'){
                		tr.append($("<td class=\"nowrap\"></td>").text('只升不降'));
                	}else if(obj.afterLevelStatus=='00'){
                		tr.append($("<td class=\"nowrap\"></td>").text('不升不降'));
                	}else{
                		tr.append($("<td class=\"nowrap\"></td>").text(''));
                	}
                	
                	tr.append($("<td class=\"nowrap\"></td>").text(obj.stffNo));
                	tr.append($("<td class=\"nowrap\"></td>").text(obj.sysId));
                	
                	tr.append($("<td class=\"nowrap\"></td>").text(obj.operateHisDate));
