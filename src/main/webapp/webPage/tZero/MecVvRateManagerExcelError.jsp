...
<body class="page-header-fixed page-sidebar-closed-hide-logo page-content-white page-md" style="background-color:white;">
   ...
   <%--  <jsp:include page="../common/header.jsp" flush="true"/> --%>
		                                        ...
		                                        <div class="row">
		                                            <div class="col-md-offset-1 col-md-11">
		                                                <button type="button" id="exportexcel_btn" class="btn btn btn-primary"> <i class="fa fa-sign-out"></i> 导出excel</button>
		                                           		 <button type="button" id="exportexcel_cancel_btn" class="btn red"> <i class="fa fa-share"></i> 确定</button>	
		                                            </div>
		                                        </div>
		                                    </div> 
		                                </div>
    ...
    $(function(){
    	...
    	data = $.parseJSON(data);
    	console.log(data);
    	$("#message").html(getInfo(data.successCount,data.failureCount,""));
      ...
      $.each(data.errors, function(index, obj) { 
		...
		$("#exportexcel_btn").on('click', function(){
			window.location.href=baseURL+'/tZero/vvRate/resultExportExcel?batchNumber='+data.batchNumber;
...
function getInfo(successCount,failureCount,msg) {
	var info = "本次批量导入操作共成功" +successCount 
			+ "条，失败" + failureCount + "条";
	return info;
}
