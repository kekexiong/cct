...
<body class="page-header-fixed page-sidebar-closed-hide-logo page-content-white page-md" style="background-color:white;">
   ...
   <%--  <jsp:include page="../common/header.jsp" flush="true"/> --%>
		                                        ...
		                                        <div class="row">
		                                            <div class="col-md-offset-1 col-md-11">
		                                                <button type="button" id="exportexcel_btn" class="btn btn btn-primary"> <i class="fa fa-sign-out"></i> 导出excel</button>
		                                           		 <button type="button" id="exportexcel_cancel_btn" class="btn red"> <i class="fa fa-share"></i> 取消</button>	
		                                            </div>
		                                        </div>
		                                    </div> 
		                                </div>
    ...
    $(function(){
    	...
    	//加载表格数据
      $.each(data.msgInfo, function(index, obj) { 
    	  var tr = $("<tr></tr>");
        	tr.append($("<td></td>").text(start + index + 1)); 
        	tr.append($("<td class=\"nowrap\"></td>").text(obj.mno)); 
        	tr.append($("<td class=\"nowrap\"></td>").text(obj.erroMsg));
		...
		$("#exportexcel_btn").on('click', function(){
			window.location.href=baseURL+'/busRate/batchResultExportExcel?batchId='+data.batchId;
			parent.$.fancybox.close();
		});
		
		//关闭页面
		$("#exportexcel_cancel_btn").on('click',function(){
			parent.$.fancybox.close();
			var url = baseURL+"/busRate/remvebatchResult";
			$.ajax({ 
	            type: "POST", 
	            url: url, 
		...
		var titileWain='本次批量导入操作即日付共成功<font color="red">'+data.successCount+'</font>条,失败<font color="red">'+
		data.failCount+'</font>条'+", 失败商户及原因如下:";
		//显示错误信息
