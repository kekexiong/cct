...
<body class="iframe-body">
		                            ...
		                            <div class="form-body">
		                                    <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
		                                        <div class="form-group">
		                                            <label class="control-label col-md-4 text-right">开通起止日期:</label>
		                                            <div class="col-md-8 paddingnone">
		                                                <div class="input-group input-daterange " data-date="2016/08/04" data-date-format="yyyy/mm/dd">
		                ...
		                <div class="portlet light bordered scaffold-list">
		                    ...
		                    <div class="portlet-body wheel"  >
		                        <table  class="table table-striped table-bordered table-hover table-checkable order-column" id="queryMecGrid">
		                            <thead>
		                                <tr>
		                                    <th class="nowrap"> 行号</th>
		                                 	<th class="nowrap"> 开通日期</th>
		                                    <th class="nowrap"> 合作商编号 </th>
		                                    <th class="nowrap"> 合作商名称 </th>
		                                    <th class="nowrap"> T+0业务状态 </th>
		                                    <th class="nowrap"> T+0业务规模 </th>
		                                    <th class="nowrap"> 已开展的T+0商户数</th>  
		                                </tr>
		                            </thead>
		                            <tbody>
		                            </tbody>
 ...
 <!-- Excel的模态框（Modal） -->
	<div class="modal fade" id="exportExcelWin" tabindex="-1" data-backdrop="static" data-width="400px">
    ...
    //查询数据
    function reloadGrid() {
		var url=baseURL+"/tZero/org/query";
		...
		param.dtOpenBg = $('#dtOpenBg').val().replace('/', '').replace('/', '').trim(); //开始时间
		param.dtOpenEd   = $('#dtOpenEd').val().replace('/', '').replace('/', '').trim(); //结束时间
		...
		$.ajax({
            ...
            success: function(data) {
                ...
                $.each(data.items, function(index, obj) {  
                	...
                	trData.push(obj.dtOpen);
                	trData.push(obj.orgNo);
                	trData.push(obj.orgNm);
                	trData.push(obj.stlSts);
                	trData.push(obj.limitAmt);
                	trData.push(obj.mecCount);
    ...
    $(function(){
    	...
    	/*初始化页面按钮权限*/
    	if (!hasPermission(PERMISSSIONCONST.TZERO301)) {$("#query_btn").hide();}
    	if (!hasPermission(PERMISSSIONCONST.TZERO302)) {$("#exportexcel_btn").hide();}
		...
		$("#exportexcel_btn").on('click', function() {
			bootbox.confirm("确定要导出excel?",function(btn){
				if(btn){
					window.open(baseURL + "/tZero/org/epexcel"); 
				}
			})
